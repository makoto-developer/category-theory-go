...
インスタンス（データ）は、この圏から集合の圏への**関手**です。各対象に行の集合を、各射に行どうしの写像を割り当てます。
...
|---:|---|---:|---:|---:|
| 100,000 | 外部キーの全域性 | 14,846,181 | 4,729,704 | 517 |
| 100,000 | パス等式 | **6,944,638** | **0** | **0** |

**2.14倍、しかも確保ゼロ。**

理由は素直です。全域性の検査は「行き先が実在するか」を見るので、対象の行を `map` に組む必要があります。パス等式の検査は**写像を辿って比べるだけ**なので、何も確保しません。

「高級な制約は高くつく」と思いがちですが、逆でした。
...
| n | 方向 | ns/op | B/op | allocs/op | 行数 |
|---:|---|---:|---:|---:|---:|
| 4,000 | Δ（引き戻し） | **2** | **0** | **0** | n |
| 4,000 | Σ（和） | 6,551 | 131,072 | 1 | 2n |
| 4,000 | Π（積） | 8,173,421 | 256,000,003 | 1 | n² |

**Δ は確保ゼロ**——スライスを共有するだけです。$\Sigma$ は線形（n を4倍にすると確保量もちょうど4倍）、$\Pi$ は二次（ちょうど16倍）。

n=4,000 で **$\Pi$ は $\Sigma$ の 1,248倍の時間、1,953倍の確保量**。

**随伴の形が、そのまま確保量に出ています。** 左随伴は余極限を作るので足し算、右随伴は極限を作るので掛け算。「どちら向きの移行が安いか」を、実装を見る前に言えます。
