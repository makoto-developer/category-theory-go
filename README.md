...
| [`part6/`](part6/) | 発展 | F余代数と unfold。hylomorphism の中間構造は自動では消えない。手で消してもアロケーションが減るだけで、速さは Go のバージョンと書き方の組み合わせで変わる |
| [`part7/`](part7/) | 発展 | レンズ。3則を全部通るレンズが元の値を書き換える。直そうとすると Get/Set 表現では1段につき 65,536 バイト重なるが、Mod 表現に変えると 48 バイトで済む |
| [`part10/`](part10/) | 発展 | 圏論的データベース。スキーマは圏の表示でデータは関手。移行は Σ ⊣ Δ ⊣ Π の3方向で、離散スキーマなら Σ は線形・Π は二次。パス等式は単独の外部キー制約では書けず、検査は追加確保ゼロ |
| [`part8/`](part8/) | 発展 | 極限と余極限。引き戻しは（条件つきで）INNER JOIN。結合順を変えても答えは同じでコストは133倍違う。filter の押し下げが安全かは、述語がキー写像を通って因子化するかで決まる |
| [`part9/`](part9/) | 発展 | 余モナド。移動平均は extend そのもの。法則どおり duplicate を実体化するとメモリが 4.99倍。そしてレンズは Store 余モナドの余代数だった |
