...
|---|---:|---:|---:|
| 法則どおり（fmap f ∘ duplicate） | 772,215 | 4,005,889 | 2 |
| extend を直接書く | 342,984 | 802,816 | 1 |
| 手書きループ | **247,589** | 802,816 | 1 |

**2.25倍遅く、メモリは 4.99倍。** part6 で見た hylomorphism の中間構造とまったく同じ形の話です。

そして直接版と手書きループは**メモリが同じ**で、時間だけ 1.39倍。抽象化の代金はクロージャの呼び出しだけです。
...
**要素あたり 32.03 バイト。$O(n)$ です。**

`Zipper` は「スライスヘッダ24B + 位置 int 8B」で 32B。`Duplicate` は `Items` を**共有する** `Zipper` を $n$ 個作るだけなので、配列そのものは1本しかありません。深くコピーしていたら n=100,000 で 80GB です。

**part7 で「Go の浅いコピーは危険だ」と書きました。同じ性質が、ここでは `duplicate` を安くしています。** 危険なのは性質そのものではなく、性質と要件の食い違いです。
